...
public interface ArchetypeRepository extends JpaRepository<Archetype, UUID> {
    ...
    Archetype findByCharacterListName(String characterName);

    @Query("Select a FROM Archetype a JOIN FETCH a.characterList c JOIN FETCH c.element")
    List<Archetype> findAllWithCharactersAndElements();
}