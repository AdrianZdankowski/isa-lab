...
public class CharacterController {
    ...
    @GetMapping("/api/archetypes/{archetypeId}/characters")
    public ResponseEntity<?> getCharactersByArchetype(@PathVariable UUID archetypeId) {
        List<GetCharactersResponse.CharacterInfo> characterInfoList = characterService.findAllByArchetype(archetypeId).stream()
                .map(character -> GetCharactersResponse.CharacterInfo.builder()
                        .id(character.getId())
                        .name(character.getName())
                        .build())
    ...
    @GetMapping("/api/elements/{elementId}/characters")
    public ResponseEntity<?> getCharactersByElement(@PathVariable UUID elementId) {
        List<GetCharactersResponse.CharacterInfo> characterInfoList = characterService.findAllByNatureElement(elementId).stream()
                .map(character -> GetCharactersResponse.CharacterInfo.builder()
                        .id(character.getId())
                        .name(character.getName())
                        .build())
    ...
    public ResponseEntity<?> addCharacter(@RequestBody PostCharacterRequest request) {
        ...
        if (request.getName() == null || request.getPowerLevel() == null || request.getArchetypeId() == null) {
        ...
        UUID defaultElementId = UUID.fromString("83bdf2ce-ae92-428f-bda3-d5838053e2b7");

        Optional<NatureElement> optionalElement = natureElementService.getNatureElementById(defaultElementId);
